...
.PHONY: all
all: depend $(LIB_DIST_NATIVE) $(LIB_DIST_BYTECODE)
...
$(LIB_DIST_NATIVE): $(SOURCES_OBJ_NAT)
	@ mkdir -p $(LIB_FOLDER)
	$(OCAMLFIND) $(OCAMLOPT) $(FOLDERS_OPT) $(OCAMLFIND_FLAGS) \
		-a -o $@ $(COMPILATION_ORDER)

$(LIB_DIST_BYTECODE):  $(SOURCES_OBJ_BYT)
	@ mkdir -p $(LIB_FOLDER)
	$(OCAMLFIND) $(OCAMLC) $(FOLDERS_OPT) $(OCAMLFIND_FLAGS) \
		-a -o $@ $(COMPILATION_ORDER:.cmx=.cmo)
...
INSTALL = META $(SOURCES_ML) $(SOURCES_MLI) \
	$(SOURCES_OBJ_BYT) $(SOURCES_OBJ_NAT) $(SOURCES_CMI) \
	$(LIB_DIST_NATIVE) $(LIB_DIST_BYTECODE) $(LIB_DIST).a
